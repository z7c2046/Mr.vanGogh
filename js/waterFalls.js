...
function waterfalls(father, son) {
    ...
    for (let i = 0; i < innerContainer.length; i++) {
        boxHeight =
            innerContainer[i].offsetHeight +
            parseInt(getStyle(innerContainer[0], "margin-bottom")); //盒子高度-----------后面+20是通过css给box设置的下边margin
        //2.3 判断是否是第一行
        if (i < columns) {
    ...
    var maxBoxHeight = parseInt(
        //获取最后一个元素的top值
        innerContainer[innerContainer.length - 1].style.top
    );

    var lastBoxHeight =
        innerContainer[innerContainer.length - 1].offsetHeight +
        parseInt(
